import type { SimulationResult, SimStats, GoldenAnalysis, ComparisonResult } from '../types.ts';
import type { ExpansionCollectionState, RarityState } from './collection.ts';
...
function simulateMultiExpansionRun(
  ...
  const states = collections.map((c) => cloneState(c, 0));
...
function addNormalCardMeta(s: SimState, ms: MetaSimState, rarity: Rarity): void {
  if (rarity === 'legendary') {
    if (s.legendaries.unowned > 0) {
      if (ms.legendaries > 0 && rng() < ms.legendaries / s.legendaries.unowned) {
        ms.legendaries--;
  ...
  const roll = rng() * incomplete;
  if (roll < pool.at0) {
    if (metaAt0 > 0 && rng() < metaAt0 / pool.at0) {
    ...
    if (metaAt1 > 0 && rng() < metaAt1 / pool.at1) {
