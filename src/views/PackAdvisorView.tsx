...
import { DUST_COST, RARITY_COLORS } from '../types.ts'
...
function scoreAggregatePack(
  ...
  return {
    code: '__aggregate__',
    name: label,
    isStandard: label === 'Standard Pack',
    isAggregate: true,
...
export default function PackAdvisorView() {
  ...
  const scores = useMemo(() => {
    ...
    const expScores = filteredExpansions
      .map(exp => scoreExpansion(exp, bySet.get(exp.code) ?? [], allMissingBySet.get(exp.code) ?? []))
      .filter(s => s.missingMetaCards > 0)

    const aggregateLabel = formatFilter === 'standard' ? 'Standard Pack' : 'Wild Pack'
    ...
    const aggregate = scoreAggregatePack(aggregateLabel, filteredExpansions, filteredMetaBySet, filteredAllBySet)
    ...
    return result
  }, [getEnrichedCards, expansions, formatFilter, selectedClass, selectedRarities, collection, metaStandard, metaWild, collectionMode, sortCol, sortAsc, variantConfirmed])
  ...
  return (
    <div className="p-6 max-w-5xl">
      ...
      {scores.length === 0 ? (
        <div className="bg-white/5 rounded-lg border border-white/10 p-8 text-center text-gray-400">
          No missing meta cards in {formatFilter} format. You have everything!
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10 text-gray-400 text-xs select-none">
                ...
                <th className="text-left px-4 py-3">Pack Type</th>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="text-white font-medium flex items-center gap-2">
                        {!score.isAggregate && <ExpansionPackIcon code={score.code} size={28} />}
                      ...
                      {score.craftCost > 0 && score.craftCost <= 1600 && (
                      ...
                      {isExpanded && score.topCards.length > 0 && (
                        <div className="mt-3 border-t border-white/5 pt-2">
                          <div className="flex items-center gap-2 text-[10px] text-gray-500 mb-1.5 font-medium uppercase tracking-wide">
                            ...
                            <span className="w-14 text-right">Craft</span>
                          ...
                          {score.missingMetaCards > score.topCards.length && (
                            <div className="text-[10px] text-gray-500 pt-1">
                              ... {score.missingMetaCards - score.topCards.length} more cards not shown
                            </div>
                          )}
                        </div>
                      )}
      ...
      <div className="mt-4 space-y-1.5">
        <p className="text-[10px] text-gray-600">
          <span className="text-gray-500">Meta / Pack</span> — expected meta-relevant dust saved per pack.
          Weights each missing card by played %, craft cost, and pull chance (with duplicate protection).
        </p>
        <p className="text-[10px] text-gray-600">
          <span className="text-gray-500">Avg / Pack</span> — expected dust value per pack. New cards save their craft cost, duplicates give disenchant value.
        </p>
        <p className="text-[10px] text-gray-600">
          <span className="text-gray-500">Score</span> — per-card meta value: pull chance &times; played % &times; craft cost. Higher means this card contributes more expected value per pack.
        </p>
      </div>
    </div>
  )
